        ...
        for (const [_channel, feed] of FEEDS) {
            ...
            try {
                ...
                contents.items.forEach((item) => {
                    ...
                    const dateStr = `${pubDate.getFullYear()}.${month < 10 ? `0${month}` : month}.${
                        date < 10 ? `0${date}` : date
                    }`;

                    if (!videos[dateStr]) videos[dateStr] = [];

                    if (feed.includes("youtube")) {
                        videos[dateStr].push(parseYouTubeItem(item, dateStr, contents));
                    } else {
                        videos[dateStr].push(parseLexFridmanItem(item, dateStr));
                    }
                });
            } catch (e) {
                console.error(e);
...
const parseYouTubeItem = (item, dateStr, contents) => ({
    ...item,
    dateStr,
...
const parseLexFridmanItem = (item, dateStr) => ({
    ...item,
    dateStr,
    youtube: item.link,
    link: item.link,
